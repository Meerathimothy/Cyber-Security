...
First we enter into msf6 using msfconsole command. For port scanning, nmap is being used. All open ports are counted.
...
Here First we enter into msf6 using msfconsole. Now we use search options with scanner or netbios and use option which have both folder. Now we will set RHOST with machine ip. And we will run with run command. We will get it in unix names.
...
Portscanning is done using nmap. sV -P will give port number, its state , service version . port 8000 has been specified. Name and version of service regarding port 8000 is shown.
...
First we search smb folders. Since we need password , will search smb_login. So search command with SMB or SMB_logins
We will use appropriate option.

![image](https://github.com/Meerathimothy/Cyber-Security/assets/57287429/1d2cfa65-45ef-49a0-84e5-c7d7c9ca08df)

Setting SMB user. set DB_ALL_PASS true and we set USERPASSFILE_FILE 'file location' and run the exploit. Sucess state give password.
...
Here smtp folder is search . Use the proper folder . Name is given in smtp relay folder.
...
Now we will do hashdump we will get all the users id, name and password hash value. From that pirate password hash is taken
...
First of all set smbuser as ballen , smbpwd as password1. set rhost with machine ip and lhost with terminal's ip. After that exploit command is runned and entered into meterpreter
...
By giving sysinfo we will get the name.
